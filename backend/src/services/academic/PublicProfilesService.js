...
class PublicProfilesService {
    ...
    async prepareSandboxPublishPayload(userId, options = {}) {
        console.log(`🔵 [PublicProfilesService.prepareSandboxPublishPayload] Preparando payload sandbox: ${userId}`);
        ...
        console.log(`🟢 [PublicProfilesService.prepareSandboxPublishPayload] Payload preparado`, {
            advancedPdfCount: enrichedEntries.length
        });
    ...
    async prepareAdvancedPdfPayload(userId, cycleId, selectedRosterName) {
        console.log(`🔵 [PublicProfilesService.prepareAdvancedPdfPayload] Preparando PDF avancado: ${userId}`);
    ...
    async enrichDisciplinesWithCatalogData(disciplines = [], cache = new Map()) {
        ...
        await Promise.all(uniqueCodes.map(async (codigo) => {
            ...
            try {
                ...
                console.warn(`🟡 [PublicProfilesService] Falha ao buscar disciplina ${codigo} no catalogo`, error?.message);
