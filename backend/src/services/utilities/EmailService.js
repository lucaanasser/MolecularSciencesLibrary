...
class EmailService {
    ...
    /**
     * Notifica todos os administradores ativos sobre um novo pedido de cadastro.
     * @param {Object} params
     * @param {Object} params.user - Dados do usuário que solicitou cadastro
     */
    async sendRegistrationRequestNotification({ user }) {
        try {
            ...
            const subject = 'Nova solicitação de cadastro - Biblioteca CM';

            const htmlContent = `
                <p>Olá, administrador!</p>
                <p>Um novo usuário solicitou cadastro na <strong>Biblioteca Ciências Moleculares</strong> e aguarda sua aprovação.</p>
                ...
                <div style="text-align: center; margin: 20px 0;">
                    <a href="${adminUrl}" style="background: #b657b3; color: #fff; padding: 12px 28px; border-radius: 6px; text-decoration: none; font-weight: bold; font-size: 16px;">Ver no painel de administração</a>
                </div>
                <p style="font-size: 13px; color: #555;">Acesse: Painel Admin → Usuários → <strong>Solicitações de Cadastro</strong> para aprovar ou rejeitar.</p>
            `;

            const textContent = `Nova solicitação de cadastro\n\nNome: ${user.name}\nNUSP: ${user.NUSP}\nEmail: ${user.email}\nTelefone: ${user.phone}\nTurma: ${user.class || '-'}\n\nAcesse o painel de administração para aprovar ou rejeitar:\n${adminUrl}`;
            ...
            console.log(`🟢 [EmailService] Notificação de cadastro enviada para: ${ADMIN_EMAIL}`);
            return true;
        } catch (error) {
