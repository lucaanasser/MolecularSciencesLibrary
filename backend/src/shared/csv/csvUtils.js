// Utilitario generico para importacao/exportacao de CSV

/**
 * Escapa valores para CSV (adiciona aspas se necessario)
 */
...
/**
 * Funcao generica para importar entidades via CSV
 */
async function importFromCSV({ fileBuffer, requiredFields, mapRow, addFn, logger }) {
    const csvContent = fileBuffer.toString('utf-8');
    const lines = csvContent.split('\n').filter(line => line.trim());
    if (lines.length < 2) {
        return { success: 0, failed: 0, errors: [{ row: 0, error: 'Arquivo CSV vazio ou invalido' }] };
    }
