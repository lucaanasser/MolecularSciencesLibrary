...
export const ProfileAvatar = ({ name, profileImage, isOwnProfile, isEditing, avatarTimestamp }: ProfileAvatarProps) => {
  ...
  return (
    <div className="p-6 bg-gradient-to-b from-library-purple/5 to-white">
      <div className="relative mx-auto w-48 h-48 mb-4">
        <Avatar className="w-full h-full border-4 border-white shadow-lg">
          {avatarSrc ? (
            <AvatarImage src={avatarSrc} alt={name} />
          ) : null}
          <AvatarFallback className="bg-library-purple text-white text-6xl font-bebas">
            {name?.charAt(0)}
          </AvatarFallback>
        </Avatar>
        {isOwnProfile && isEditing && (
          <button className="absolute bottom-2 right-2 w-10 h-10 bg-library-purple text-white rounded-full shadow-lg hover:bg-library-purple/90">
