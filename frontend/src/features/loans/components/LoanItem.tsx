    ...
    <div className="border-b border-gray-100 pb-4 last:border-0 last:pb-0">
      <div className="flex justify-between items-start">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 text-sm text-gray-500">
            <div className="flex items-center">
              ...
              <span>Empréstimo: {formatDate(loan.borrowed_at)}</span>
            </div>
            {loan.due_date && (
              <div className="flex items-center">
                <Clock className="mr-1 h-3 w-3" />
            ...
            {loan.returned_at && (
              <div className="flex items-center">
                <Clock className="mr-1 h-3 w-3" />
                <span>Devolução: {formatDate(loan.returned_at)}</span>
              </div>
            )}
        ...
        <div className="flex flex-col items-end gap-2">
          {showRenew && (
            <button
              className="flex items-center gap-2 bg-library-purple text-white px-4 py-2 rounded hover:bg-library-purple-muted disabled:opacity-50"
              onClick={onRenew}
