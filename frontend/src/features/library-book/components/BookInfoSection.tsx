...
const BookInfoSection: React.FC<BookInfoSectionProps> = ({ book, books, loanByBook, highlightColor }) => {
    ...
    <div>
      <ul className="flex flex-col gap-1 text-gray-700">
        <li className="flex items-center gap-2">
          <Users className="w-4 h-4" style={{color: highlightColor}}/>
          <span className="font-medium">Autores:</span>
          <span className="truncate">{book.authors}</span>
        </li>
        {!!book.volume && (
          <li className="flex items-center gap-2">
            <Layers className="w-4 h-4" style={{color: highlightColor}} />
            <span className="font-medium">Volume:</span>
            <span>{book.volume}</span>
          </li>
        )}
        <li className="flex items-center gap-2">
          <Globe className="w-4 h-4" style={{color: highlightColor}} />
          <span className="font-medium">Idioma:</span>
          <span>{book.language}</span>
        </li>
        <li className="flex items-center gap-2">
          <BookOpen className="w-4 h-4" style={{color: highlightColor}} />
          <span className="font-medium">Área:</span>
          <span>{book.area}</span>
        </li>
        <li className="flex items-center gap-2">
          <Hash className="w-4 h-4" style={{color: highlightColor}} />
          <span className="font-medium">Subárea:</span>
          <span>{book.subarea}</span>
        </li>
        {donatorDisplay && (
          <li className="flex items-center gap-2">
            <Gift className="w-4 h-4" style={{color: highlightColor}} />
