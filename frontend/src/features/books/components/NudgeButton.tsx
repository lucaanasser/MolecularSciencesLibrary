...
const NudgeButton: React.FC<NudgeButtonProps> = ({ book }) => {
  ...
  return (
    <div className="flex flex-col items-end">
      <button
        className={`group/nudge flex items-center justify-start transition-all duration-200 bg-white text-white font-semibold rounded-full px-0 w-8 h-8 overflow-hidden hover:bg-cm-purple hover:text-white hover:px-4 hover:w-auto focus-visible:bg-cm-purple focus-visible:text-white focus-visible:px-4 focus-visible:w-auto focus:outline-none disabled:opacity-60 disabled:cursor-not-allowed`}
      ...
      >
        <span className="flex items-center justify-start pl-2 w-full h-full">
          <svg
            ...
            viewBox="0 0 24 24"
            className="w-5 h-5 flex-shrink-0 transition-colors duration-200 text-cm-purple group-hover/nudge:text-white group-focus/nudge:text-white fill-current"
          >
