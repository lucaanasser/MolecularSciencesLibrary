...
import { useGenericEvaluation, CardService } from ".";
...
interface RatingCardProps<TAggregated, TEvaluation> {
  entityId: number;
  isLoggedIn: boolean;
  criterios: Crit[];
  service: CardService<TAggregated, TEvaluation>;
  onLogin: () => void;
  onOpenForm: (myEvaluation: TEvaluation | null) => void;
}

export default function RatingCard<TAggregated, TEvaluation>({
  entityId,
  isLoggedIn,
  criterios,
  service,
  onLogin,
  onOpenForm,
}: RatingCardProps<TAggregated, TEvaluation>) {
  const {
    aggregatedRatings,
    myEvaluation,
    isLoading,
    handleDelete,
  } = useGenericEvaluation<TAggregated, TEvaluation>({
    entityId,
    isLoggedIn,
    ...service,
  });

  // Adapte para o formato esperado pelo layout
  const mediaAvaliacoes = aggregatedRatings
    ? Object.fromEntries(
        criterios.map((c) => [c.key, (aggregatedRatings as any)[`media_${c.key}`] || 0])
      )
    : null;
  const totalAvaliacoes = (aggregatedRatings as any)?.total_avaliacoes || 0;
        ...
        {isLoggedIn ? (
          myEvaluation ? (
            <div className="flex gap-2">
              ...
              <Button onClick={() => handleDelete((myEvaluation as any).id)} variant="destructive">
