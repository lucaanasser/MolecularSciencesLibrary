...
CREATE TABLE donators (
            ...
            contact TEXT,
            notes TEXT,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, tag TEXT,
            FOREIGN KEY(user_id) REFERENCES users(id),
            FOREIGN KEY(book_id) REFERENCES books(id)
        );
...
CREATE TABLE forum_questions (
            ...
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            is_anonymous INTEGER DEFAULT 0, is_pinned INTEGER DEFAULT 0, disciplina_codigo TEXT,
...
CREATE TABLE international_experiences (
            ...
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, avancado_id INTEGER,
            FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE
        );
...
CREATE INDEX idx_forum_questions_disciplina ON forum_questions(disciplina_codigo);
...
CREATE INDEX idx_forum_comments_target ON forum_comments(target_type, target_id);
CREATE INDEX idx_forum_subscriptions_question ON forum_subscriptions(question_id);
